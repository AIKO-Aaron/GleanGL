...
SandBox::RayCaster::RayCaster(SandBox::scene s) {
	cameraOut_color = Glean::OpenCL::KernelFunction<cl_float4, cl_float2, cl_float2, cl_mem, cl_int, cl_mem, cl_int, cl_mem, cl_float>("assets/opencl/raycast_out.cl", "getColors", 2, new size_t[2]{ SCREEN_WIDTH * PIXEL_ACCURACY, SCREEN_HEIGHT * PIXEL_ACCURACY });
...
void SandBox::RayCaster::render() {
	cameraOut_color(cameraPos, cameraAngle, screenSize, buffer, scene.numObjects, sceneBuffer, scene.numLights, lightsBuffer, curTime += 0.01f);
